...
wrapperFcn = function (graph, divId) {
    ...
    drawVis = function (graph, divID) {
        ...
        circles = d3.select(divID).selectAll(".node");

        links = d3.select(divID).selectAll(".link");

        annotations = d3.select(divID).select("g#annotation");

        highlight();
        tooltip();
        filterDpt();

        d3.select(divID).on('click', clearHighlights)
    }
    ...
    vis = function (graph, divID) {
        ...
        //add toggle button if true; 

        if (graph.toggle){
            let toggle_button = svg_internal.append('g');

            toggle_button.append('text')
            .attr('class', 'toggleLabel')
            .text('Temas de Trabalho')
            .attr('x',width/2)
            .style('text-anchor','start')
            .style('fill','#3d3d3d')
            .on('click',function(){
                d3.selectAll('.toggleLabel').style('font-weight','normal') 

                d3.select(this).style('font-weight','bolder') 

                d3.select(divID).selectAll(".link")
                .style('visibility','visible')
                let toHide = d3.select(divID).selectAll(".link").filter(d=>d.type == 'aspiringTheme')
            ...
            toggle_button.append('text')
            .attr('class', 'toggleLabel')
            .text('Temas que Gostaria de Trabalhar')
            .attr('x',width/2)
            .style('fill','#a6a6a6;')
            .attr('y',20)
            .style('text-anchor','start')
            .on('click',function(){
                ...
                d3.select(divID).selectAll(".link")
                .style('visibility','visible')
                let toHide = d3.select(divID).selectAll(".link").filter(d=>d.type == 'theme')
                toHide.style('visibility','hidden')

            })
        

            toggle_button.attr('transform','translate(0,20)')
        }
        ...
        const link = svg_internal.append("g")
            ...
            .style('stroke-width',d=> (d.count + 'px') || '')
    ...
    filterDpt = function () {
        ...
        circles.on("click", function (d) {
            ...
            if (d3.event.shiftKey) {
                ...
                links.classed('hide', l => (l.type !== 'internal' || l.source.department !== d.department));
...
themes = function (data, temas_grouping) {
            ...
            if (person.themes) {
                person.themes.map(theme => {
                    ...
                    if (sourcePerson && targetArea) {
                        let edge = { source: sourcePerson.id, target: targetArea.id, type: 'theme', count:1 };
                        //add to neighborList
                        ...
                        if (existingEdge) {
                            existingEdge.count = existingEdge.count +1;
                        } else {
                            edges.push(edge);
                        }
                ...
                //create edges between people and their target areas
                allPeople.map(person => {
                    if (person.aspiringThemes) {
                        person.aspiringThemes.map(theme => {
                            let sourcePerson = person
                            let targetArea = allPeople.find(t => t.name === theme);
        
                            if (sourcePerson && targetArea) {
                                let edge = { source: sourcePerson.id, target: targetArea.id, type: 'aspiringTheme', count:1 };
                                //add to neighborList
                                sourcePerson.neighbors.push(targetArea);
                                targetArea.neighbors.push(sourcePerson);
    ...
    return { nodes: allPeople, links: edges, toggle:true }
}
